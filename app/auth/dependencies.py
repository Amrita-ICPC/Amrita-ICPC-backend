...
from app.exceptions.auth import UnauthorizedError, RoleMissingError, PermissionDeniedError
    ...
    def __call__(self, user: Dict[str, Any] = Depends(get_current_user)):
        ...
        if self.allowed_groups:
            ...
            if not has_group:
                ...
                has_group = any(group in normalized_groups for group in self.allowed_groups)
        ...
        if self.allowed_groups or self.allowed_roles or self.permission:
            err_msg = []
            if self.allowed_groups: err_msg.append(f"groups={self.allowed_groups}")
            if self.allowed_roles: err_msg.append(f"roles={self.allowed_roles}")
            if self.permission: err_msg.append(f"perm={self.permission}")
            
            logger.warning(f"Access denied for user {user.get('preferred_username')}. Missing: {', '.join(err_msg)}")
...
def can_create(resource: str): return Depends(check_permission(resource, "create"))
def can_read(resource: str): return Depends(check_permission(resource, "read"))
def can_update(resource: str): return Depends(check_permission(resource, "update"))
def can_delete(resource: str): return Depends(check_permission(resource, "delete"))
