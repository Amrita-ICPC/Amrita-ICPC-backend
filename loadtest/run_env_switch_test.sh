...
if [[ -f "$TUNNEL_ENV_FILE" ]]; then
    # Generated by open_tunnels.sh. Keep this before defaults so HOST and
    # KEYCLOAK_SERVER_URL from the tunnel file become the normal local values.
    # Do not clobber explicit caller-provided env: HOST=... ./run_env_switch_test.sh
    # must still win over the convenience tunnel file.
    while IFS='=' read -r key value; do
        case "$key" in
            HOST) HOST="${HOST:-$value}" ;;
            KEYCLOAK_SERVER_URL) KEYCLOAK_SERVER_URL="${KEYCLOAK_SERVER_URL:-$value}" ;;
        esac
    done < "$TUNNEL_ENV_FILE"
    export HOST KEYCLOAK_SERVER_URL
    echo "Loaded tunnel env from $TUNNEL_ENV_FILE"
fi
